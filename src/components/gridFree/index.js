...
var 
    ...
    */
    _cache = {
        ...
        isInit: true,
        fragment: {
            loading: '<div class="l-gridFree-loading"><div class="l-gridFree-loadingBg"></div><div class="l-gridFree-loadingIco"></div></div>',
    ...
    */
    _core = {
        ajax: function(callback){
            ...
            if( !_cache.isInit ){
                cookie.set(pathname, args, 200000);
            }
            if( /getGridPrev/.test(location.search) ){
                args = cookie.get(pathname) ? cookie.get(pathname) : args;
            ...
            $.ajax({
                ...
                success: function(data){
                    _cache.data = data;
                    _cache.tmpData[p.pageIndex - 1] = data.lists;
        ...
        */
        ...
        before: function(){
            var that = this,
                data = tool.isFunction( p.before ) ? p.before() : null;

            if( data ) _cache.data.selectedData = data;
        },

        after: function(){
            if( tool.isFunction(p.after) ) p.after();
        }
    },
    ...
    */
    GridFree = function(options){
        ...
        /*if( p.data ){
            _cache.data = p.data;
            _cache.tmpData[p.pageIndex - 1] = p.data.lists;
            _core.createBody();
            _core.createFooter();
            _cache.isInit = false;
        }else{*/
...
* @param {Number} i 当前页的行索引
*/
GridFree.prototype.setlistselected = function(selectedData){
    _cache.listselected.push(selectedData);
};
...
* @param {Number} i 当前页的行索引
*/
GridFree.prototype.dellistselected = function(selectedData){
    var listselected = _cache.listselected,
        len         = listselected.length,
        i           = 0,
        index       = 0;

    for(; i<len; i++){
        var isEqual = equal(listselected[i], selectedData);

        if( isEqual ){
            _cache.listselected.splice(i, 1);
        }
    }
    //console.log( _cache.listselected );
};
...
* @param {Number} i 当前页的行索引
*/
GridFree.prototype.getlistselected = function(i){
    return _cache.listselected;
};
