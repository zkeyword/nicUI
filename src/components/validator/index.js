        ...
        c = {
            ...
            rule: {
                ...
                ip: function(sVal, oItem){
                    ...
                    if( !/^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/i.test(sVal) ){
                        return c.handleText(oItem, 'ip');
                    }
                },
            ...
            run: function(){
                var oThat      = this,
                    ...
                    fAction    = function(oSelf){
                                    ...
                                    if( tool.isFunction(fRules) && sRule === 'process' ){
                                        return processHandle(sRule, fRules(oSelf), true );
                                    }
                                    
                                    if( tool.isFunction(fAjax) && sRule === 'ajax' ){
                                        processHandle(sRule);
